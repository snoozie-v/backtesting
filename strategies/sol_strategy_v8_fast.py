...
- R-based partial profit-taking: 30% at 1R, 30% at 2R, 30% at 3R, 10% runner
...
- Exits: R-based partials (1R/2R/3R) + ATR trailing on runner

Risk Model:
- Position sized so stop loss (-1R) = risk_per_trade_pct of account
- 1R = ATR * atr_fixed_mult (fixed stop distance from entry)
- Stop ratchets up: breakeven at 1R, +1R at 2R, +2R at 3R
...
class SolStrategyV8Fast(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.current_stop = None      # Current stop price (ratchets up)
    ...
    def _check_exits(self):
        ...
        for i, (r_mult, fraction) in enumerate(self.risk_mgr.partial_schedule):
            ...
            if target_price and current_price >= target_price:
                ...
                if partial_size > 0:
                    ...
                    print(f"[{dt}] PARTIAL {self.partials_taken}/3 @ {current_price:.4f} "
                          f"(+{pnl_r:.0f}R, selling {fraction:.0%}) | "
        ...
        if current_price <= self.current_stop:
            ...
            reason = "STOP" if self.partials_taken == 0 else f"RATCHET STOP (after {self.partials_taken}R)"
            print(f"[{dt}] EXIT {reason} @ {current_price:.4f} | "
                  f"PnL: {pnl_pct:+.1f}% ({r_mult:+.1f}R)")
        ...
        if self.partials_taken >= 3:
            ...
            # Use the higher of ratchet stop and trail stop
            effective_stop = max(self.current_stop, trail_stop)
            if current_price <= effective_stop:
                pnl_pct = (current_price - self.entry_price) / self.entry_price * 100
                r_mult = self.risk_mgr.calculate_r_multiple(
                    self.entry_price, current_price, self.stop_distance, 'long')
                print(f"[{dt}] EXIT RUNNER TRAIL @ {current_price:.4f} | "
                      f"PnL: {pnl_pct:+.1f}% ({r_mult:+.1f}R) | HWM: {self.high_water_mark:.4f}")
                self.sell(size=self.position.size)
                self._reset_state()
